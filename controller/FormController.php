   ...
   //**************** verifies form entries */
    ...
    public function registerForm(array $data)
    {
        ...
        if (empty($messages['errors'])) {
            $this->_user->addUser($teamName, $password);
   ...
   //**************** manages the login form and the creation of cookies (empty or incorrect fields)*/
    ...
    public function addGeoCacheInDB(array $data){
        ...
        if (empty($messages['errors'])) {
            ...
            $idGeoCache = $geoCache->addGeoCache($geoCacheName, $lat, $lng,$resp1,$resp2,$resp3,$goodResp);
    ...
    public function updateGeoCacheInDB(array $data){
       ...
       if (empty($messages['errors'])) {
        // if no error message, add GeoCache in bdd
        $geoCache = new GeoCache();
        $idGeoCache = $geoCache->updateGeoCache($idGeoCache, $geoCacheName, $lat, $lng,$resp1,$resp2,$resp3,$goodResp);
    ...
    public function checkingTheAnswers(array $data){
        ...
        // Gif for points
        if($UserPoints == 0){
            $messages['errors'][] = '<img src="https://giphy.com/gifs/playboyfragrances-3o7TKr3nzbh5WgCFxe/giphy.gif" 
